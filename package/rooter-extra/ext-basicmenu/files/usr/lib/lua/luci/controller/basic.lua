...
function index()
	...
	entry({"admin", "basic", "gethotspot"}, call("action_gethotspot"))
	...
	entry({"admin", "basic", "setpin"}, call("action_setpin"))
...
function action_getslide()
	...
	if file == nil then
		...
		rv['secret'] = string.sub(secret,1,10)
...
function action_setpin()
	local rv = {}
	local set = luci.http.formvalue("set")
	os.execute("/usr/lib/basic/setpin.sh \"" .. set .. "\"")
end
