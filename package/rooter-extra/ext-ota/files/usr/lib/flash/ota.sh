...
flash() {
	...
	if [ -s /etc/zemail ]; then
		gr_backup=`grep "^/etc/zemail" /etc/sysupgrade.conf`
		[ -z $gr_backup ] && echo "/etc/zemail" >> /etc/sysupgrade.conf
	fi
	...
	curl -l -k $slist $usern > /tmp/firmware.bin
	if [ $? = '0' ]; then
...
un=$(uci -q get otaconfig.ota.key)
usern=$(echo -n "$un" | base64 --decode)
server="sftp://$ip/OTA/"
mak=$(cat /tmp/sysinfo/model)
model=$(echo $mak | tr " " "-")
full=0
if [ -e /usr/sbin/mwan3track ]; then
	full=1
fi
source /etc/codename
...
if [ "$model" = "WorldTraveller" ]; then
	...
	if [ ! -z "$rmod" ]; then
		serverx="WorldTraveller1602/"
	fi
	rmod=$(echo "$mak" | grep "oolite")
	if [ ! -z "$rmod" ]; then
		serverx="WorldTraveller826Q/"
	fi
	rmod=$(echo "$mak" | grep "wg1608")
	if [ ! -z "$rmod" ]; then
		serverx="WorldTraveller1608/"
	fi
fi
flist=$(curl -k -l $server$serverx $usern)
if [ ! -z "$flist" ]; then
	img=""
	idate=""
